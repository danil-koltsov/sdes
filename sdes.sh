...
ls1L=(${keyL[@]:1:4}); ls1R+=(${keyR[@]:1:4}) #<
ls1L[5]=${keyL[0]}; ls1R[5]=${keyR[0]}
ls1=(${ls1L[@]}); ls1+=(${ls1R[@]})
echo $(permutation "${p8[*]}" "${ls1[*]}")
}
...
ls1L[5]=${keyL[0]}; ls1R[5]=${keyR[0]}
ls1=(${ls1L[@]}); ls1+=(${ls1R[@]})
ls2L=(${ls1L[@]:2:4}); ls2R=(${ls1R[@]:2:4}) #<<
ls2L+=(${ls1L[@]:0:2}); ls2R+=(${ls1R[@]:0:2})
ls2=(${ls2L[@]}); ls2+=(${ls2R[@]})
echo $(permutation "${p8[*]}" "${ls2[*]}")
}
function round {
...
txtRound=($(round "${txtIp[*]}" "${key1[*]}")) #Round 1
txtRoundRotate=(${txtRound[@]:4:8}); txtRoundRotate+=(${txtRound[@]:0:4}) #Rotate
txtRound=($(round "${txtRoundRotate[*]}" "${key2[*]}")) #Round 2
...
key2=($(keyGeneration2 "${keyInput[*]}")) #KeyGen 2
txtIp=($(permutation "${ip[*]}" "${txtInput[*]}")) #Ip
txtRound=($(round "${txtIp[*]}" "${key2[*]}")) #Round 1
txtRoundRotate=(${txtRound[@]:4:8}); txtRoundRotate+=(${txtRound[@]:0:4}) #Rotate
txtRound=($(round "${txtRoundRotate[*]}" "${key1[*]}")) #Round 2
echo $(permutation "${ip1[*]}" "${txtRound[*]}") #Ip-1
}
txt=(0 1 1 0 1 1 0 1)
